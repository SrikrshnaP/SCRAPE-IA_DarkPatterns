...
brw.runtime.onMessage.addListener(
    function (message, sender, sendResponse) {
        ...
        } else if ("action" in message && message.action == "buttonClicked") {
            // console.log(message.product)
            dict[message.product[0]] = message.product[1]
            console.log(dict)
            for (k in dict) {
        ...
        } else if ("action" in message && message.action == "DeleteEle") {
            // console.log(message.product)
            key = message.product
            console.log("delete")
            console.log(key)
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.message === "capture_and_detect") {
      ...
      fetch("http://127.0.0.1:5001/screenshot_yolo", {
        method: "POST",
      })
