...
chrome.runtime.sendMessage({ message: "capture_and_detect" });
...
window.addEventListener('scroll', () => {
  ...
  if (currentScrollPosition > previousScrollPosition && currentScrollPosition - previousScrollPosition >= 300) {
    console.log("Scrolled down ${currentScrollPosition - previousScrollPosition}px");
...
async function initPatternHighlighter() {
  ...
  if (activationState.isEnabled === true) {
    ...
    if (url.includes("/dp/") || url.includes("/product/") || url.includes("/gp/product/") || /\/(B0|ASIN|dp)\/[0-9A-Z]{10,}/.test(url)) {
      temp = document.getElementsByClassName("a-price aok-align-center")[0]
      prodPrice = temp.querySelector(".a-price-whole").innerText
      // prodPrice = mode(price)
      prodName = document.getElementById("productTitle").innerText
      document.addEventListener("click", function(event) {
        // Check if the clicked element has the ID "add-to-cart-button"
        console.log("Button clicked");
        if (event.target && event.target.id === "add-to-cart-button") {
          // If the clicked element is the "add to cart" button, handle the event
          console.log("Button clicked");
          // Send a message to the extension
          brw.runtime.sendMessage({action: "buttonClicked", product: [prodName, prodPrice]});
          // brw.runtime.sendMessage({action: "Dictionary", dict: dict}) // do for delete if this works
            
          }
      });
    }
    cart_regex = /^https?:\/\/(?:www\.)?amazon\.(?:com|in|co\.uk|ca|de|fr|es|it|com\.au|com\.mx|com\.br|nl|ae|sg|se|tr|pl|sa|jp|cn)\/(?:gp\/cart|cart)\/?/
    if (cart_regex.test(url)) {
      deleteButtons = document.querySelectorAll('[data-action="delete"]')
      deleteButtons.forEach(button => {
        button.addEventListener("click", event => {
          // Access information about the clicked button
          const clickedButton = event.target;
          val = clickedButton.getAttribute("aria-label")
          brw.runtime.sendMessage({action: "DeleteEle", product: val.substring(7, val.length)})
