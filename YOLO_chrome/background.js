...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "captureScreenshot") {
      chrome.tabs.captureVisibleTab((image) => {
        ...
        console.log("called")
        // Handle security and permission checks (see detailed steps below)
        if (true) {
          const link = document.createElement("a");
          link.href = `data:image/${image.slice(-5)}` //+ base64Image;
          link.download = `screenshot_${Date.now()}.${image.slice(-5)}`;
          link.click();
  
          sendResponse({ saved: true });
        } else {
          sendResponse({ saved: false, error: "Permission denied or security check failed" });
        }
      });
    }
  });
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
    ...
    if (message.action === "getActivationState") {
        console.log("called background.js")
      chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        // Trigger screenshot capture when a page is entered
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.message === "capture_and_detect") {
      // Send API call to Flask server
      fetch("http://127.0.0.1:5000/screenshot_yolo", {
        method: "POST",
      })
        .then((response) => response.json())
        .then((data) => {
          // Process the response (bounding boxes, download URL, etc.)
          console.log(data); // Example logging, modify for your needs
          result = data.results
          if(result!="" || result!='')
          {
            console.log("here")
